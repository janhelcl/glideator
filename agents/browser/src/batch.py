...
def submit_batch_and_download_raw(
    ...
    *,
    ...
    # Convert Path to string for the upload API (docs show string path)
    file_path_str = str(tmp_file)
    
    uploaded_file = client.files.upload(
        file=file_path_str,
        config=types.UploadFileConfig(display_name=tmp_file.stem, mime_type=upload_mime_type)
    )
...
def run_iterative_batch(
    ...
    *,
    model: str = "gemini-2.5-pro",
    batch_config: Dict[str, Any] | None = None,
    upload_mime_type: str = "text/plain",
...
def parse_result_google_text(result: dict) -> Tuple[Any, dict] | None:
    """Extract (key, dict with text and usageMetadata) from a Google batch result line.

    Returns None if the expected text is missing.
    The returned dict contains:
      - "text": the response text
      - "usageMetadata": the usage metadata dict from the response
    """
    try:
        key = result.get("key")
        ...
        return key, {"text": text, "usageMetadata": usage_metadata}
    except Exception:
        return None
                ...
                "contents": [{
                    "parts": [{
                        "text": prompt_template.safe_substitute(
                            site_details=utils.format_site_details(row["name"], row["country"], engine),
